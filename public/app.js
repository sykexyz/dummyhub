...
function createVideoCard(video, index) {
  ...
  playButton.addEventListener("click", () => { if (!video.url || $("video", frame)) return; const player = document.createElement("video"); player.className = "video-player"; player.controls = true; player.preload = "metadata"; player.setAttribute("controlsList", "nodownload noplaybackrate noremoteplayback"); player.setAttribute("disablePictureInPicture", "true"); player.disableRemotePlayback = true; player.src = video.url; $(".video-art", frame).classList.add("hidden"); $(".frame-grid", frame).classList.add("hidden"); playButton.classList.add("hidden"); frame.appendChild(player); player.play().catch(() => {}); });
